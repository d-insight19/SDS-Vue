        ...
        <section
          ...
          <div class="day-wrap">
            <div
              ...
              <span
                :class="{restday: day.restday, rest: day.rest, workday: day.workday}"
                class="number">{{ day.text }}</span>
  ...
  methods: {
      ...
      if (this.custom.length) {
        ...
        for (let i = this.customIndex, j = 0; j < this.custom.length; i++) {
          ...
          if (v.text) {
            ...
            else throw new Error(`custom数组每项的类型应该是String或Object，但是得到的是${type}`)
          }
        }
      } else {
        this.months.forEach(v => { days = days.concat(v.days) })
    ...
    setData (data, date) {
      ...
      if (target.length) {
        ...
        throw new Error(`Date format error.Got "${date}".It's should be like "YYYY-MM-DD".`)
      }
    },
    constructionBody () {
      const months = []
...
.content main .aki-month .day-wrap {
  display: flex;
  flex-wrap: wrap;
}
.content main .aki-month .day-wrap .day {
  display: flex;
  flex-direction: column;
...
.content main .aki-month .day-wrap .day.disabled {
  color: #eee;
}
.content main .aki-month .day-wrap .day.disabled > span {
  color: #eee!important;
}
.content main .aki-month .day-wrap .day.select {
  color: white;
  background-color: deepskyblue;
  box-shadow: none;
}
.content main .aki-month .day-wrap .day.select > span {
  color: white!important;
}
.content main .aki-month .day-wrap .day.active {
  background-color: lightblue;
  box-shadow: none;
}
.content main .aki-month .day-wrap .day span {
  font-size: 12px;
}
.content main .aki-month .day-wrap .day span.number {
  font-size: 14px;
}
.content main .aki-month .day-wrap .day span.rest {
  color: tomato;
}
.content main .aki-month .day-wrap .day span.restday:before {
  content: '休';
  position: absolute;
  top: 2px;
  left: 2px;
...
.content main .aki-month .day-wrap .day span.workday:before {
  content: '班';
  position: absolute;
  top: 2px;
  left: 2px;
