<template>
 <!-- eslint-disable -->
    <!-- wrap -->
    <div id="wrap" class="colorCode2">

        <!-- container -->
        <div id="container" class="mediaquery sample_page">
            <div id="content" class="list">
                <div class="cell">
                    <div class="inner">
                        <h2> L.2. Thumbnail list_vertical view</h2>
                        <br/>

                        <div>
                        <p> 메인 포틀릿 </p>
                        <ul class="thumList">
                            <li>
                                <div class="in">
                                    <a href="#" class="thum">
                                        <span class="label">정규</span>
                                        <div class="thumbnail">
                                            <img src="@/assets/img/thum_no_s.jpg" class="thumImg" alt="">
                                        </div>
                                    </a>
                                    <a href="#" class="tit">만화로 배우는 알기 쉬운 경영전</a>
                                    <ul class="info">
                                        <li class="color_clr3 fwb">이러닝</li>
                                        <li><span class="star ico"></span>4.5</li>
                                    </ul>
                                    <div class="tagWrap"><span>교재</span><span>모바일병행</span></div>
                                </div>
                            </li>
                            <li>
                                <div class="in">
                                    <a href="#" class="thum">
                                        <span class="label">정규</span>
                                        <div class="thumbnail">
                                            <img src="@/assets/img/thum_no_s.jpg" class="thumImg" alt="">
                                        </div>
                                    </a>

                                    <a href="#" class="tit">[경기] 빅데이터 전문가 과정(기술) - 테스트</a>
                                    <ul class="info">
                                        <li class="color_clr3 fwb">이러닝</li>
                                        <li><span class="star ico"></span>4.5</li>
                                    </ul>
                                    <div class="tagWrap"><span>교재</span><span>모바일병행</span></div>
                                </div>
                            </li>
                            <li>
                                <div class="in">
                                    <a href="#" class="thum">
                                        <span class="label">정규</span>
                                        <div class="thumbnail">
                                            <img src="@/assets/img/thum_no_s.jpg" class="thumImg" alt="">
                                        </div>
                                    </a>

                                    <a href="#" class="tit">[경기] 빅데이터 전문가 과정(기술) - 테스트</a>
                                    <ul class="info">
                                        <li class="color_clr3 fwb">이러닝</li>
                                        <li><span class="star ico"></span>4.5</li>
                                    </ul>
                                    <div class="tagWrap"><span>교재</span><span>모바일병행</span></div>
                                </div>
                            </li>
                            <li>
                                <div class="in">
                                    <a href="#" class="thum">
                                        <span class="label">정규</span>
                                        <div class="thumbnail">
                                            <img src="@/assets/img/thum_no_s.jpg" class="thumImg" alt="">
                                        </div>
                                    </a>

                                    <a href="#" class="tit">[경기] 빅데이터 전문가 과정(기술) - 테스트</a>
                                    <ul class="info">
                                        <li class="color_clr3 fwb">이러닝</li>
                                        <li><span class="star ico"></span>4.5</li>
                                    </ul>
                                    <div class="tagWrap"><span>교재</span><span>모바일병행</span></div>
                                </div>
                            </li>
                        </ul>
                        <!-- //list -->
                        </div>

                        <br/>
                        <div>
                        <p> 구독 </p>
                        <!-- list -->
                        <ul class="thumList type5">
                            <li>
                            <div class="in">
                                <a href="#" class="thum">
                                    <div class="thumbnail">
                                        <img src="@/assets/img/thum_no_s.jpg" class="thumImg" alt="">
                                    </div>
                                </a>
                                <a href="#" class="tit">초보를 위한 업무에 바로 쓰는 자바 스크립트 입문 과정</a>
                                <div class="writer">홍길동</div>
                                <ul class="info">
                                    <li class="icon">
                                        <span class="view ico"></span><span class="info_value">2,190</span>
                                    </li>
                                    <li class="icon">
                                        <span class="like ico"></span><span class="info_value">36</span>
                                    </li>
                                    <li class="icon">
                                        <span class="time ico"></span><span class="info_value">17:32</span>
                                    </li>
                                </ul>
                            </div>
                            </li>
                            <li>
                            <div class="in">
                                <a href="#" class="thum">
                                    <span class="label2">맛보기</span>
                                    <div class="thumbnail">
                                        <img src="@/assets/img/thum_no_s.jpg" class="thumImg" alt="">
                                    </div>
                                </a>

                                <a href="#" class="tit">채널명이 한 줄인 경우</a>
                                <div class="writer">홍길동</div>
                                <ul class="info">
                                    <li class="icon">
                                        <span class="heart ico"></span><span class="info_value">2,190</span>
                                    </li>
                                    <li class="icon">
                                        <span class="like ico"></span><span class="info_value">36</span>
                                    </li>
                                    <li class="icon">
                                        <span class="time ico"></span><span class="info_value">17:32</span>
                                    </li>
                                </ul>
                                
                            </div>
                            </li>
                            
                        </ul>
                        <!-- //list -->
                        </div>


                        <br/>
                        <div>
                        <p> 도서리스트 </p>
                        <!-- list -->
                        <ul class="thumList type6">
                            <li>
                            <div class="in">
                                <a href="#" class="thum">
                                    <div class="thumbnail">
                                        <img src="@/assets/img/thum_no_s.jpg" class="thumImg" alt="">
                                    </div>
                                </a>
                                <a href="#" class="tit">초보를 위한 업무에 바로 쓰는 자바 스크립트 입문 과정</a>
                                <ul class="info">
                                    <li class="icon">
                                        <span class="view ico"></span><span class="info_value">2,190</span>
                                    </li>
                                    <li class="icon">
                                        <span class="like ico"></span><span class="info_value">36</span>
                                    </li>
                                    <li class="icon">
                                        <span class="time ico"></span><span class="info_value">17:32</span>
                                    </li>
                                </ul>
                            </div>
                            </li>
                            <li>
                            <div class="in">
                                <a href="#" class="thum">
                                    <div class="thumbnail">
                                        <img src="@/assets/img/thum_no_s.jpg" class="thumImg" alt="">
                                    </div>
                                </a>

                                <a href="#" class="tit">채널명이 한 줄인 경우</a>
                                <ul class="info">
                                    <li class="icon">
                                        <span class="heart ico"></span><span class="info_value">2,190</span>
                                    </li>
                                    <li class="icon">
                                        <span class="like ico"></span><span class="info_value">36</span>
                                    </li>
                                    <li class="icon">
                                        <span class="time ico"></span><span class="info_value">17:32</span>
                                    </li>
                                </ul>
                                
                            </div>
                            </li>
                            
                        </ul>
                        </div>
                        <!-- //list -->


                        <br/>
                        <div>
                        <p>구독</p>
                        <!-- list -->
                        <ul class="thumList type7">
                            <li>
                                <div class="in">
                                    <a href="#" class="tit">초보자를 위한 업무에 바로 쓰는 자바 스크립트 입문 과정</a>
                                    <a href="#" class="writer">홍길동</a>
                                    <ul class="info">
                                        <li>게시물 10</li>
                                        <li>구독 550</li>
                                        <li><span class="line">P</span> 5</li>
                                    </ul>
                                </div>
                                <div class="thum">
                                    <div class="cDim04"></div>
                                    <div class="cDim03"></div>
                                    <div class="thumbnail">
                                        <img src="@/assets/img/thum_no_L2.jpg" class="thumImg" alt="">
                                    </div>
                                </div>
                            </li>
                            <li>
                                <div class="in">
                                    <a href="#" class="tit">초보자를 위한 업무에 바로 쓰는 자바 스크립트 입문 과정</a>
                                    <a href="#" class="writer">홍길동</a>
                                    <ul class="info">
                                        <li>게시물 10</li>
                                        <li>구독 550</li>
                                        <li><span class="line">P</span> 5</li>
                                    </ul>
                                </div>
                                <div class="thum">
                                    <div class="cDim04"></div>
                                    <div class="cDim03"></div>
                                    <div class="thumbnail">
                                        <img src="@/assets/img/thum_no_L2.jpg" class="thumImg" alt="">
                                    </div>
                                </div>
                            </li>
                            <li>
                                <div class="in">
                                    <a href="#" class="tit">초보자를 위한 업무에 바로 쓰는 자바 스크립트 입문 과정</a>
                                    <a href="#" class="writer">홍길동</a>
                                    <ul class="info">
                                        <li>게시물 10</li>
                                        <li>구독 550</li>
                                        <li><span class="line">P</span> 5</li>
                                    </ul>
                                </div>
                                <div class="thum">
                                    <div class="cDim04"></div>
                                    <div class="cDim03"></div>
                                    <div class="thumbnail">
                                        <img src="@/assets/img/thum_no_L2.jpg" class="thumImg" alt="">
                                    </div>
                                </div>
                            </li>
                        </ul>
                        </div>
                        <!-- //list -->
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- //wrap -->
</template>

<script>
export default {
  name: 'components_thumblist_vertical',
  /* vue lifecycle */
  created () {
  },
  mounted () {
  },
  /* vue data */
  data () {
    return {
    }
  },
  /* vue function */
  methods: {
  }
}
</script>

<style>
</style>
