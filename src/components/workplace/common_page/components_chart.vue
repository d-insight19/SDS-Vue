<template>
 <!-- eslint-disable -->
    <!-- wrap -->
    <div id="wrap" class="colorCode2">

        <!-- container -->
        <div id="container" class="mediaquery sample_page">
            <div id="content" class="list">
                <div class="cell">
                    <div class="inner">
                        <h2> charts </h2>
                        <br/>

                        <div>
                        <p> basic bar </p>
                        <!-- basic bar -->
                        <h2 class="label-tit16 w100p">분류별 접수 현황</h2>
                        <ul class="progBox_wrap">
                            <li>
                                <p>신청/취소<span class="fr">101명</span> </p>  
                                <div class="progBox">
                                    <div class="bar3" style="width: 30%; height:8px; border-radius: 100px;"></div>                            
                                </div>
                            </li>
                            <li>
                                <p>학습<span class="fr">180명</span> </p>  
                                <div class="progBox">
                                    <div class="bar2" style="width: 50%; height:8px; border-radius: 100px;"></div>                            
                                </div>
                            </li>
                            <li>
                                <strong>평가<span class="fr">245명</span> </strong>  
                                <div class="progBox">
                                    <div class="bar4" style="width: 70%; height:8px; border-radius: 100px;"></div>                            
                                </div>
                            </li>
                            <li>
                                <p>수료<span class="fr">297명</span></p>  
                                <div class="progBox">
                                    <div class="bar1" style="width: 80%; height:8px; border-radius: 100px;"></div>                            
                                </div>
                            </li>
                            <li>
                                <p>기타<span class="fr">124명</span> </p>  
                                <div class="progBox">
                                    <div class="bar5" style="width: 40%; height:8px; border-radius: 100px;"></div>                            
                                </div>
                                <p class="desc">기타의견 : 0 명</p>
                            </li>
                        </ul>
                        <!-- //basic bar -->
                        </div>
                        <br/>

                        <div>
                        <p> basic bar2 </p>
                        <!-- basic bar2 -->
                        <h2 class="label-tit18 w100p">월별 학습시간</h2>
                        
                        <ul class="diagram_wrap type2 tar">
                            <li>이러닝</li>
                            <li class="type4">지식</li>
                            <li class="type5">집합</li>             
                        </ul>
                        <ul class="progBox_wrap">
                            <li>
                                <p>1월<span class="fr">총 10시간 30분</span></p>
                                <div class="progBox">
                                    <div class="bar1" style="width: 40%; height:8px; border-radius: 100px;"></div>
                                </div>
                                <div class="progBox">
                                    <div class="bar4" style="width: 10%; height:8px; border-radius: 100px;"></div>
                                </div>
                                <div class="progBox">
                                    <div class="bar5" style="width: 50%; height:8px; border-radius: 100px;"></div>
                                </div>
                            </li>
                            <li>
                                <p>2월<span class="fr">총 2시간 30분</span></p>
                                <div class="progBox">
                                    <div class="bar1" style="width: 40%; height:8px; border-radius: 100px;"></div>
                                </div>
                                <div class="progBox">
                                    <div class="bar4" style="width: 10%; height:8px; border-radius: 100px;"></div>
                                </div>
                                <div class="progBox">
                                    <div class="bar5" style="width: 50%; height:8px; border-radius: 100px;"></div>
                                </div>
                            </li>
                            <li>
                                <p>3월<span class="fr">총 2시간 30분</span></p>
                                <div class="progBox">
                                    <div class="bar1" style="width: 40%; height:8px; border-radius: 100px;"></div>
                                </div>
                                <div class="progBox">
                                    <div class="bar4" style="width: 10%; height:8px; border-radius: 100px;"></div>
                                </div>
                                <div class="progBox">
                                    <div class="bar5" style="width: 50%; height:8px; border-radius: 100px;"></div>
                                </div>
                            </li>
                        </ul>
                        <!-- //basic bar2 -->
                        </div>
                        <br/>

                        <div>
                        <p> basic bar3 </p>
                        <!-- basic bar3 -->
                        <div>
                            <p class="inner-tit">당신의 의견은 어떻습니까?</p>
                            <p class="person">참여인원 <span>50</span>명</p>
                        </div>
                        <div class="progBox mt22">
                            <div class="bar clr1" style="width: 17%; height:8px; border-radius: 100px;"></div>                            
                        </div>
                        <div class="txt-wrap clearfix">
                            <p class="prog-txt">대통령의 가족을 지켜는 법은 필수이기 때문에 현재 정책을 유지해야 한다.</p>
                            <div class="fr">
                                <span class="part-num"><span>5</span>명</span>
                                <span class="pipe"></span>
                                <span class="part-num-2"><span>17</span>%</span>
                            </div>                                   
                        </div>
                        <div class="progBox mt22">
                            <div class="bar clr2" style="width: 58%; height:8px; border-radius: 100px;"></div>                            
                        </div>
                        <div class="txt-wrap clearfix">
                            <p class="prog-txt"> <span class="chip border_clr2 color_clr2">나의 선택</span>  대통령의 가족도 우리와 같은 시민일 뿐, 경호 예우는 필요없다.</p>
                            <div class="fr">
                                <span class="part-num"><span>16</span>명</span>
                                <span class="pipe"></span>
                                <span class="part-num-2"><span>58</span>%</span>
                            </div>                                  
                        </div>
                        <div class="progBox mt22">
                            <div class="bar clr3" style="width: 21.5%; height:8px; border-radius: 100px;"></div>                            
                        </div>
                        <div class="txt-wrap clearfix">
                            <p class="prog-txt">잘 모르겠다.</p>
                            <div class="fr">
                                <span class="part-num"><span>7</span>명</span>
                                <span class="pipe"></span>
                                <span class="part-num-2"><span>21.5</span>%</span>
                            </div>                                
                        </div>
                        <!-- //basic bar3 -->
                        </div>
                        <br/>

                        <div>
                        <p> basic bar4 </p>
                        <!-- basic bar4 -->
                        <div class="txt-wrap clearfix">
                            <p class="fl good-ex"><span class="ico agree"></span> 찬성</p>
                            <p class="fr bad-ex">반대</p>
                        </div>
                        <div class="progBox bg-x">                           
                            <div class="bar l-bar" style="width: 63.5%; height:8px; "></div>
                            <div class="bar r-bar" style="width: 36%; height:8px; "></div>                            
                        </div>                        
                        <div class="txt-wrap clearfix">
                            <p class="prog-txt"></p>
                            <div class="fl">
                                <span class="part-num-2"><span>64</span>%</span>                                
                                <span class="pipe"></span>
                                <span class="part-num"><span>32</span>명</span>
                            </div>    
                            <div class="fr">
                                <span class="part-num"><span>18</span>명</span>
                                <span class="pipe"></span>
                                <span class="part-num-2"><span>36</span>%</span>
                            </div>                                  
                        </div>
                        <!-- //basic bar4 -->
                        </div>
                        <br/>

                        <div>
                        <p> stacked bar </p>
                        <!-- stacked bar -->
                        
                        <!-- //stacked bar -->
                        </div>
                        <br/>

                        <div class="MainType">
                        <p> basic donut </p>
                        <!-- basic donut -->
                        
                        <div class="status">
                            <div class="white big">
                                <div class="info">
                                    <img src="@/assets/img/thum_no_s.jpg" class="" alt="">
                                </div>
                                <dl>
                                    <dt>취득학점</dt>
                                    <dd><strong>59</strong> 학점</dd>
                                </dl>
                                <dl class="line">
                                    <dt>목표학점</dt>
                                    <dd><strong>80</strong> 학점</dd>
                                </dl>
                            </div>
                        </div>
                        <!-- //basic donut -->
                        </div>
                        <br/>

                        <div>
                        <p> stacked donut </p>
                        <!-- stacked donut -->
                        <h2 class="label-tit20 w100p">Q. 질문 1 입니다.</h2>
                        <ul class="diagram_wrap">
                            <div class="diagram2">
                                <img src="@/assets/img/thum_no_s.jpg" class="" alt="">
                            </div>
                            <li>
                                <p>직무과정<span class="percent fr"><span class="num">36</span>%(52명)</span></p>  
                            </li>
                            <li class="type4">
                                <p>경영 리더십<span class="percent fr"><span class="num">25</span>%(12명)</span></p>  
                            </li>
                            <li class="type5">
                                <p>외국어 과정<span class="percent fr"><span class="num">28</span>%(36명)</span></p>  
                            </li>
                            <li class="type2">
                                <p>금융과정<span class="percent fr"><span class="num">14</span>%(8명)</span></p>  
                            </li>
                            <li class="type3">
                                <p>교육과정<span class="percent fr"><span class="num">5</span>%(6명)</span></p>  
                            </li>                           
                        </ul>
                        <!-- //stacked donut -->
                        </div>
                        <br/>

                        <div>
                        <p> column </p>
                        <!-- column -->
                        <!-- //column -->
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- //wrap -->
</template>

<script>
export default {
  name: 'components_chart',
  /* vue lifecycle */
  created () {
  },
  mounted () {
  },
  /* vue data */
  data () {
    return {
    }
  },
  /* vue function */
  methods: {
  }
}
</script>

<style>
</style>
