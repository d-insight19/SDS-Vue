...
if (support.touch || isIeMobile) {
   ...
   flowplayer(function(player, root) {
      ...
      var hasMoved = false;
      bean.on(root, 'touchmove', function(e) {
        hasMoved = true;
      });
...
flowplayer(function(api, root) {
  ...
  function selectQuality(quality) {
    common.find('.fp-qsel-menu a', root).forEach(function(el) {
      var currQuality = -1;
      if(currQuality > -1){
          common.toggleClass(el, 'fp-selected', el.getAttribute('data-quality') == currQuality);
          common.toggleClass(el, 'fp-color', el.getAttribute('data-quality') == currQuality);
      }else{
...
flowplayer(function(p, root) {
  ...
  p.targetSubtitles = function(i) {

      var url = loSubtitles[i].src;
      if (!url) return;
      setActiveSubtitleClass(i);
      ...
      var $target = $("#subtitles");
      $target.empty();
      common.xhrGet(url, function(txt) {
        var entries = p.conf.subtitleParser(txt);
        entries.forEach(function(entry) {
            var html = "";
            html += "<li id='subtitle"+entry.title +"'>";
            html += " <a href='#' onclick='fnSeekTime(" + entry.startTime + ")'>";
            html += " <span class=time>" + format(entry.startTime) + "</span>" 
            html += " <span class='desc'>" + entry.text + "</span></a>";
            html += "</li>";
            $target.append(html);
        });
      }, function() {
...
flowplayer(function(api, root) {
  ...
  try {
   ...
   var ui = common.createElement('div', {className: 'fp-ui'}, '\
         ...
         <div class="fp-header">\
           <a class="fp-previcon"></a>\
           <p class="fp-header-txt"></p>\
           <a class="fp-setting"></a>\
           <a class="fp-closeicon"></a>\
           <a class="fp-videotag">맛보기</a>\
         </div>\
         <p class="fp-speed-flash"></p>\
         <div class="fp-play fp-visible" style="display:none">\
         ...
         <div class="fp-pause" style="display:none;">\
           <a class="fp-icon fp-playbtn"></a>\
           {{ PAUSE_SHARP_OUTLINE }}\
           {{ PAUSE_SHARP_FILL }}\
         ...
         <div class="fp-nextArrow"/>\
          <p class="fp_Icon_txt">홍보영상</p>\
         </div>\
         </div>\
         <div class="fp-finishIconWrap"/>\
          <div class="fp-playIconWrap"/>\
          <svg class="autoplay_cicle" viewbox="0 0 33.83098862 33.83098862" width="64" height="64" xmlns="http://www.w3.org/2000/svg">\
            <circle class="autoplay_cicle_background" stroke="#fff" stroke-width="2" fill="none" cx="16.91549431" cy="16.91549431" r="15.91549431" style="stroke-opacity:0.3;" />\
            <circle class="autoplay_cicle_circle" stroke="#00abfb" stroke-width="2" stroke-dasharray="100,100" stroke-linecap="round" fill="none" cx="16.91549431" cy="16.91549431" r="15.91549431" />\
          </svg>\
            <div class="fp-autoplayIcon"/></div>\
          </div>\
          <div class="fp-nextcontents-tit"/>다음 콘텐츠:모바일 게임 UI 디자인 실무</div>\
          <div class="fp-cancleBtn"/>취소</div>\
         </div>\
		 <div class="fp-presec">\
         </div>\
         <div  class="fp-nextsec">\
         </div>\
         <div class="mini_progress">\
              <div class="current_progree mask_clr2"></div>\
          </div>\
         <div class="fp-controls">\
          <div class="fp-controls-inner">\
              <a class="fp-icon fp-playbtn"></a>\
              <div class="fp-volume">\
              ...
              <span class="fp-repeat"></span>\
              <a class="fp-script"></a>\
              <a class="fp-fullscreen"></a>\
              <a class="fp-studyplan"></a>\
            </div>\
  ...
  var waiting = find('waiting'),
   ...
   bean.on(root, "click.player", function(e) {
    ...
    //  if (common.hasClass(e.target, 'fp-ui') || common.hasClass(e.target, 'fp-engine') || e.flash || common.hasParent(e.target, '.fp-play,.fp-pause,.fp-presec,.fp-nextsec')) {
     if (common.hasClass(e.target, 'fp-engine') || e.flash || common.hasParent(e.target, '.fp-play,.fp-pause,.fp-presec,.fp-nextsec')) {
  ...
  // 더블텝 영역 확인테스트
  bean.on(root, 'click', '.half_left', function() {
    console.log("왼쪽영역 터치 확인")
  });
  // 더블텝 영역 확인테스트
  bean.on(root, 'click', '.half_right', function() {
    console.log("오른쪽영역 터치 확인")
  });
   bean.on(root, 'click', '.fp-toggle, .fp-play, .fp-playbtn , .fp-playIcon , .fp-pauseIcon', function() {
     if (api.disabled) return;
     api.toggle();
   });

   /* controlbar elements */
   bean.on(root, 'click', '.fp-volumebtn', function() { api.mute(); }); 
